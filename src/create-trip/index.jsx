...
import "./../../public/a.css";
import { auth } from "@/service/firebaseConfig"; // Import auth
import { doc, setDoc } from "firebase/firestore";
import { db } from "@/service/firebaseConfig";
import { useNavigate } from "react-router-dom";
import GooglePlacesAutocomplete from "react-google-places-autocomplete";
...
function CreateTrip() {
  ...
  const SaveAiTrip = async (TripData) => {
    ...
    if (!user?.email) {
      toast("User not authenticated. Please sign in.", "error");
      setLoading(false);
      return;
    }
    const docId = Date.now().toString();
    try {
      ...
      toast("Error saving trip. Please try again.", "error");
      setLoading(false);
    }
  };
  ...
  const OnGenerateTrip = async () => {
    ...
    try {
      ...
      toast("Error generating trip. Please try again.", "error");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="sm:px-10 md:px-32 lg:px-56 xl:px-72 px-5 mt-10">
      ...
      <Dialog open={!currentUser && openDialog}>
        {" "}
        ...
        <DialogContent>
          <DialogHeader>
            <DialogDescription>
              ...
              <h2 className="font-bold text-lg mt-7">Sign In Required</h2>
              <p>Please sign in with Google to generate your trip.</p>
              <Button
                onClick={() => {
                  // Programmatically trigger the sign-in from Header
                  ...
                  if (signInButton) {
                    ...
                    toast("Sign-in button not found.", "error");
                  }
                }}
                className="mt-5 w-full flex gap-4 items-center"
              >
