...
function Activities({ trip }) {
  const activities = trip?.tripData?.tripData?.contextualRecommendations?.activities;
  ...
  return (
    <div>
      <h2 className="font-bold text-xl mt-5">Activities</h2>
      <div className="grid grid-cols-2 gap-5 mt-5 md:grid-cols-2 lg:grid-cols-3 sm:grid-cols-1">
        {activities.map((activity, index) => (
          <Card
            key={index}
            className="p-4 shadow-lg rounded-2xl bg-white justify-center items-center"
          >
